...
def anchor_exclusion_loss(probs, reg_feats, anchors, margin=0.5):
...
def train_one_epoch(model, device, loader, opt):
    ...
    for batch in loader:
        ...
        loss_cons = anchor_consistency_loss(reg_proj, txt_pool, tb["ent_types"], tb["labels"], anchors)
        loss_excl = anchor_exclusion_loss(probs, reg_proj, anchors)
        loss_margin = region_margin_loss(logits, tb["labels"], margin=0.5)
